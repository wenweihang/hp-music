...
<table class="table table-borderless table-hover">
    ...
    <tr>
        ...
        <td scope="col" id="tb-img" width="50"></td>
        <td scope="col" id="tb-song">歌曲</td>
        <td scope="col" id="tb-author">专辑</td>
        <td scope="col" id="tb-icon" width="150"></td>
        <td scope="col" id="tb-time" class="text-center">时长</td>
    </tr>
    </thead>
    <tbody id="tbody">
    <tr style="height: 15px;"></tr>
    <c:forEach items="${songsForSingerList}" var="songsForSinger" varStatus="i">
        <tr class="tr-song song${i.index}">
            ...
            <td class="ctb-img"><img src="${songsForSinger.sg_imgAddress}" width="50" height="50"></td>
            <td class="ctb-name">
                <span class="sSong-name"><a href="#">${songsForSinger.sg_name}</a></span>
                <span id="lossless">无损</span>
            </td>
            <td class="ctb-author">${songsForSinger.szj_title}</td>
            <td class="ctb-icon text-right">
                ...
                <a class="sl-add">
                    ...
                    <c:if test="${!empty logUser}">
                        <c:if test="${empty songsForSinger.collect}">
            ...
            <td class="ctb-time text-center">${songsForSinger.sg_time}</td>
            <td class="sg_songList" style="display: none;">${songsForSinger.sg_songList}</td>
            <td class="id" style="display: none;">${songsForSinger.sg_id}</td>
            <td class="musAddress" style="display: none;">${songsForSinger.sg_address}</td>
            <td class="language" style="display: none;">${songsForSinger.sg_language}</td>
            <td class="szj_title" style="display: none;">${songsForSinger.szj_title}</td>
            <td class="szj_detail" style="display: none;">${songsForSinger.szj_detail}</td>
        </tr>
    </c:forEach>
    </tbody>
</table>
<script>
    ...
    //点击歌曲跳转
    $(".sSong-name").click(function () {
        var name = $(this).text()
        var id = $(this).parent().siblings(".id").text()
        var author = $(this).parent().siblings(".author").text()
        var imgAddress = $(this).parent().siblings(".ctb-img").find("img").attr("src")
    ...
    $(".sl-download").click(function () {
        if (${empty logUser}) {
            ...
            var sg_imgAddress = $(this).parent().siblings(".ctb-img").find("img").attr("src")
    ...
    $(".sl-play").click(function () {
        ...
        if (!ap.audio.paused) {
            ...
            $(this).parent().siblings("td").find(".sSong-name>a").css("color", "rgb(190,50,50)")
            $(this).parent().parent().siblings().find("td").css("color", "")
            $(this).parent().parent().siblings().find("td>.sSong-name>a").css("color", "")
        }
    })
    //歌曲双击播放
    $(".tr-song").dblclick(function () {
        ...
        if (!ap.audio.paused) {
            ...
            $(this).children("td").find(".sSong-name>a").css("color", "rgb(190,50,50)")
            $(this).siblings().find("td").css("color", "")
            $(this).siblings().find("td>.sSong-name>a").css("color", "")
        }
    })
    //当前播放变色效果
    changeCSS()
    ...
    $(".aplayer-list ").click(function () {
        ...
        $(".tr-song").each(function (i, n) {
            ...
            if (url.indexOf(sg_name) != -1) {
                ...
                $(this).find("td>.sSong-name>a").css("color", "rgb(190,50,50)")
                $(n).siblings().find("td").css("color", "")
                $(n).siblings().find("td>.sSong-name>a").css("color", "")
            }
        })
    })
    ...
    function singleClick($this) {
        ...
        var sg_imgAddress = $this.find(".ctb-img>img").attr("src")
    ...
    function changeCSS() {
        ...
        if (ap.audio.paused) {
            ...
            $("#tbody").find("td>.sSong-name>a").css("color", "")
        } else {
            //当音乐播放条音乐播放时
            var url = decodeURI(ap.audio.src)
            $(".tr-song").each(function (i, n) {
                ...
                if (url.indexOf(sg_name) != -1) {
                    ...
                    $(n).find(".ctb-num>span").hide()
                    $(n).find("td").css("color", "rgb(190,50,50)")
                    $(n).find("td>.sSong-name>a").css("color", "rgb(190,50,50)")
                }
            })
        }
    }
