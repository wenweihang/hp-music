...
<div class="row mt-4 pb-5" id="allSongLists">
    <c:forEach items="${newSongLists}" var="newSongList" varStatus="i">
        <div class="col-md mt-3">
            <div class="card" id="zj-card">
                <a href="#" class="sl text-decoration-none" id="${i.index}">
                    <img class="card-img" src="${newSongList.sl_imgAddress}"/>
                    <div class="bt-songList-play">
                        <img class="play-img" src="<%=path%>/images/music-home/play.png">
                    </div>
                    <div id="sl_id" style="display: none">${newSongList.sl_id}</div>
                    <div id="sl_name" style="display: none;">${newSongList.sl_name}</div>
                    <div id="sl_detail" style="display: none;">${newSongList.sl_detail}</div>
                        <%--<div id="collect" style="display: none;">${allSongList.collect}</div>--%>
                </a>
                <div class="card-body">
                    <h6 class="card-title"><b>${newSongList.sl_title}</b></h6>
                    <p class="card-text">${newSongList.sl_tag}</p>
                </div>
            </div>
        </div>
    </c:forEach>
